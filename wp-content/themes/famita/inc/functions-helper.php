...
if ( ! function_exists( 'famita_body_classes' ) ) {
	function famita_body_classes( $classes ) {
		global $post;
		$show_footer_mobile = famita_get_config('show_footer_mobile', true);
		$header = apply_filters( 'famita_get_header_layout', famita_get_config('header_type') );
		if ( is_page() && is_object($post) ) {
			$class = get_post_meta( $post->ID, 'apus_page_extra_class', true );
			if ( !empty($class) ) {
				$classes[] = trim($class);
			}
			$header_layout = famita_get_header_layout();
		...
		if ( famita_get_config('preload', true) ) {
			$classes[] = 'apus-body-loading';
		}
		if ( famita_get_config('image_lazy_loading') ) {
	...
	add_filter( 'body_class', 'famita_body_classes' );
}

if ( ! function_exists( 'famita_get_shortcode_regex' ) ) {
	function famita_get_shortcode_regex( $tagregexp = '' ) {
...
if ( ! function_exists( 'famita_tagregexp' ) ) {
	function famita_tagregexp() {
		return apply_filters( 'famita_custom_tagregexp', 'video|audio|playlist|video-playlist|embed|famita_media' );
	}
}

if ( !function_exists('famita_get_header_layouts') ) {
	function famita_get_header_layouts() {
		$headers = array(
			'v1' => 'v1',
			'v2' => 'v2',
			'v3' => 'v3',
...
if ( !function_exists('famita_get_header_layout') ) {
	function famita_get_header_layout() {
		global $post;
		if ( is_page() && is_object($post) && isset($post->ID) ) {
			return famita_page_header_layout();
		}
		return famita_get_config('header_type');
	}
	add_filter( 'famita_get_header_layout', 'famita_get_header_layout' );
}

if ( !function_exists('famita_get_footer_layouts') ) {
	function famita_get_footer_layouts() {
		$footers = array();
		$args = array(
			'posts_per_page'   => -1,
			'offset'           => 0,
...
if ( !function_exists('famita_get_footer_layout') ) {
	function famita_get_footer_layout() {
		if ( is_page() ) {
			global $post;
			$footer = '';
			if ( is_object($post) && isset($post->ID) ) {
				...
				if ( empty($footer) || $footer == 'global' ) {
					return famita_get_config('footer_type', '');
				}
			}
			return $footer;
		}
	...
	add_filter('famita_get_footer_layout', 'famita_get_footer_layout');
}

if ( !function_exists('famita_blog_content_class') ) {
	function famita_blog_content_class( $class ) {
		$page = 'archive';
		if ( is_singular( 'post' ) ) {
            $page = 'single';
        }
		if ( famita_get_config('blog_'.$page.'_fullwidth') ) {
			return 'container-fluid';
		}
		return $class;
	}
}
add_filter( 'famita_blog_content_class', 'famita_blog_content_class', 1 , 1  );


if ( !function_exists('famita_get_blog_layout_configs') ) {
	function famita_get_blog_layout_configs() {
		$page = 'archive';
		if ( is_singular( 'post' ) ) {
            $page = 'single';
        }
		$left = famita_get_config('blog_'.$page.'_left_sidebar');
		$right = famita_get_config('blog_'.$page.'_right_sidebar');

		switch ( famita_get_config('blog_'.$page.'_layout') ) {
		 	case 'left-main':
	 		...
	 		case 'main':
	 			...
	 			break;
		 	default:
		 		$configs['right'] = array( 'sidebar' => 'sidebar-default',  'class' => 'col-md-4 col-sm-4 col-xs-12' ); 
...
if ( !function_exists('famita_page_content_class') ) {
	function famita_page_content_class( $class ) {
		global $post;
		if (is_object($post)) {
...
add_filter( 'famita_page_content_class', 'famita_page_content_class', 1 , 1  );

if ( !function_exists('famita_get_page_layout_configs') ) {
	function famita_get_page_layout_configs() {
		global $post;
		if ( is_object($post) ) {
...
if ( !function_exists('famita_page_header_layout') ) {
	function famita_page_header_layout() {
		global $post;
...
if ( ! function_exists( 'famita_get_first_url_from_string' ) ) {
	function famita_get_first_url_from_string( $string ) {
...
if ( !function_exists( 'famita_get_link_attributes' ) ) {
	function famita_get_link_attributes( $string ) {
...
if ( !function_exists( 'famita_post_media' ) ) {
	function famita_post_media( $content ) {
		$is_video = ( get_post_format() == 'video' ) ? true : false;
		$media = famita_get_first_url_from_string( $content );
		$media = $media['link'];
		if ( ! empty( $media ) ) {
			global $wp_embed;
			$content = do_shortcode( $wp_embed->run_shortcode( '[embed]' . $media . '[/embed]' ) );
		} else {
			$pattern = famita_get_shortcode_regex( famita_tagregexp() );
...
if ( !function_exists( 'famita_post_gallery' ) ) {
	function famita_post_gallery( $content, $args = array() ) {
		$output = '';
		$defaults = array( 'size' => 'large' );
		$args = wp_parse_args( $args, $defaults );
	    $gallery_filter = famita_gallery_from_content( $content );
...
if (!function_exists('famita_gallery_from_content')) {
    function famita_gallery_from_content($content) {
...
if ( !function_exists( 'famita_random_key' ) ) {
    function famita_random_key($length = 5) {
...
if ( !function_exists('famita_substring') ) {
    function famita_substring($string, $limit, $afterlimit = '[...]') {
        if ( empty($string) ) {
        	return $string;
        }
...
function famita_is_apus_framework_activated() {
	return defined('APUS_FRAMEWORK_VERSION') ? true : false;
}

function famita_is_cmb2_activated() {
	return defined('CMB2_LOADED') ? true : false;
}

function famita_is_woocommerce_activated() {
	return class_exists( 'woocommerce' ) ? true : false;
}

function famita_is_yith_wcwl_activated() {
...
function famita_is_yith_woocompare_activated() {
	if ( !function_exists('is_woocommerce_extension_activated') ) {
		return false;
	}
...
function famita_is_vc_activated() {
	return class_exists( 'Vc_Manager' ) ? true : false;
}

function famita_is_revslider_activated() {
	return function_exists( 'putRevSlider' );
}

function famita_is_dokan_activated() {
	return class_exists( 'WeDevs_Dokan' ) ? true : false;
}

function famita_is_wcvendors_activated() {
	return class_exists( 'WC_Vendors' ) ? true : false;
}

function famita_is_woo_swatches_activated() {
