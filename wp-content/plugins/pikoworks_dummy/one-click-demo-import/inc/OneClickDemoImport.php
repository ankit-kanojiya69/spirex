...
class OneClickDemoImport
{
	...
	public function create_plugin_page()
	{
		$this->plugin_page_setup = apply_filters('pt-ocdi/plugin_page_setup', array(
			'parent_slug' => 'pikoworks',
			'page_title'  => esc_html__('All Demo Import', 'pt-ocdi'),
			'menu_title'  => esc_html__('Demo Import', 'pt-ocdi'),
			'capability'  => 'import',
			'menu_slug'   => 'pikoworks-demo',
		));
	...
	public function display_plugin_page()
	{
		...
		<div class="wrap about-wrap theme-wrap">
			<h1><?php esc_attr_e('Welcome to Xtocky!', 'pikoworks_dummy'); ?></h1>
			<div class="about-text"><?php echo esc_html__('Xtocky is now installed and ready to use! Read below for additional information. We hope you enjoy it!', 'pikoworks_dummy'); ?></div>
			<div class="theme-logo"><span class="theme-version"><?php esc_attr_e('Version', 'pikoworks_dummy'); ?> <?php echo $theme->get('Version'); ?></span></div>
			<h2 class="nav-tab-wrapper">
				<?php
				printf('<a href="%s" class="nav-tab">%s</a>', admin_url('admin.php?page=pikoworks'), esc_html__("Welcome", 'pikoworks_dummy'));
				printf('<a href="%s" class="nav-tab">%s</a>', admin_url('admin.php?page=pikoworks-system'), esc_html__("System Status", 'pikoworks_dummy'));
				printf('<a href="javascript:void(0)" class="nav-tab nav-tab-active">%s</a>', esc_html__("Demo", 'pikoworks_dummy'));
				printf('<a href="%s" class="nav-tab">%s</a>', admin_url('plugins.php'), esc_html__("Plugins", 'pikoworks_dummy'));
				printf('<a href="%s" class="nav-tab">%s</a>', admin_url('admin.php?page=theme_options'), esc_html__("Theme Options", 'pikoworks_dummy'));
				printf('<a href="%s" class="nav-tab">%s</a>', admin_url('admin.php?page=pikowroks_currency'), esc_html__("Currency", 'pikoworks_dummy'));
				?>
			</h2>
			<div class="theme-section">
				<?php require_once PT_OCDI_PATH . 'views/plugin-page.php'; ?>
			</div>
			<p><?php echo sprintf(esc_html__('If need to remove Duplicate Demo Content after import use %s', 'pikoworks_dummy'), '<a href="https://wordpress.org/plugins/wordpress-reset/" target="_blank">' . esc_html__('WordPress Reset Plugin', 'xtocky') . '</a>'); ?></p>
			<div class="theme-thanks">
				<p class="description"><?php esc_attr_e('Thank you, we hope you to enjoy using Xtocky!', 'pikoworks_dummy'); ?></p>
			</div>
		</div>
	<?php
	...
	private function final_response()
	{
		...
		if (empty($this->frontend_error_messages)) {
			...
			$response['message'] .= sprintf(
				__('%1$s%3$sDummy Content Import Successfully!%4$s%2$s Please check your page and make sure that everything has imported correctly. If it did, you can deactivate the %3$sXtocky Dummy content%4$s plugin, because it has done its job.%5$s', 'pt-ocdi'),
